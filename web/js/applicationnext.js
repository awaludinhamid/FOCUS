...
$(document).ready(function(){
    ...
    var delimiter = "_";
    var kpiDefTopPos = $(".app-def-position").css("top");
    //	
    ...
    $("#zoom-slider #btn-zoom-in").click(function() {        
      for(i = 0; i < arrZoomPoint.length; i++) {
        if(i === currZoomPoint && i > 0) {
          currZoomPoint -= 1;
          var bodyObj = $("#body");
          //alert(bodyProp.left+","+bodyProp.top+","+bodyProp.width+","+bodyProp.height+",");
    ...
    function update(source) {
      ...
      if((navigator.userAgent.indexOf("MSIE") != -1 ) || (!!document.documentMode == true )) {//IF IE
        ...
        nodeEnter
          ...
          .html(function(d) {
            var htmlElement = "<button class='"+d.button+"' title='Show breakdown' style='padding: 1px; font-size: 10px'>"+d.name+
              "&nbsp;<span class='"+d.icon+"'></span></button>";
            if(kpiBrkdwnFlag == 0) {
              if(d == root) {
                htmlElement = htmlElement.replace("title='Show breakdown'","").replace("style='","style='cursor: not-allowed;");
              } else {
                if(kpiDetFlag == 1) {
        ...
        .on("mouseover", function(d) {
          if(hoverFlag == 0 && d.color != "cyan" && d.color != "grey") {
            ...
            tooltip
              ...
              .style("top",(1/arrZoomPoint[currZoomPoint].scale*d3.event.pageY - 1/arrZoomPoint[currZoomPoint].scale*bodyProp.top) + "px");
            linktip
              .transition()
              .duration(500)
              .style("opacity", .9);
